...
$Definition['Add limits to image upload dimensions in discussions and comments.'] = 'ディスカッションおよびコメント内にアップロードする画像のサイズに制限を加えます。';
...
$Definition['Allow links to be transformed'] = 'リンクを投稿やコメント内の埋め込み表示に変換できるようにする。例えば、YouTubeリンクは埋め込み動画に変換されます。';
$Definition['Allow users with the %s permission to change their own avatars from their profile pages in Vanilla.'] = '%sアクセス権を持つユーザーが、Vanillaのプロフィールページから自分のアバターを変更できるようにします。';
...
$Definition['Be creative! Try naming your ranks after things that the community talks about.'] = 'クリエイティブであれ！コミュニティが話題にする事柄からランクに名前を付けてみてください。';
...
$Definition['Checkboxes allow admins to perform batch actions on a number of discussions or comments at the same time.'] = 'チェックボックスを使用すると、管理者は多数のディスカッションまたはコメントに対して同時にバッチ処理を実行できます。';
...
$Definition['Garden.Profiles.Edit permission to change their own avatars from their profile pages in Vanilla.'] = 'Vanillaのプロフィールページから自分のアバターを変更する「Garden.Profiles.Edit」アクセス権。';
...
$Definition['Here are the ranks that users can achieve on your site.'] = 'ユーザーがサイトで達成できるランクは次のとおりです。これらのランクをカスタマイズし、新しいランクを追加することもできます。いくつかヒントを紹介します。';
...
$Definition['If you are new to HTML and/or CSS, here are some tutorials to get you started:'] = 'HTMLおよび/またはCSSを初めて使用する場合は、始めるためのチュートリアルをいくつか紹介します：';
...
$Definition['If you want to embed your forum, enable embedding.'] = 'フォーラムを埋め込むか、ブログにVanillaのコメントを使用したい場合は、埋め込みを有効にする必要があります。埋め込みを使用していない場合は、この設定をオフにすることをお勧めします。';
...
$Definition['It is a good idea to keep the maximum number of characters allowed in a post down to a reasonable size.'] = '投稿において許可される最大文字数を妥当なサイズに抑えることは良い考えです。';
...
$Definition['MobileLogoDescription'] = 'モバイル用バナーのロゴがサイトの上部に表示されます。';
...
$Definition['Overridden views or themehooks can have unintended side effects and are not previewed.'] = 'オーバーライドされたビューまたはテーマフックは、予期せぬ副次的影響を伴う場合があり、プレビューされません。';
...
$Definition['Recommend special ranks for admins and mods.'] = 'モデレーターと管理者のために特別なランクを設けて、コミュニティが誰が担当しているかを簡単に確認できるようにすることは良い考えです。';
$Definition['Recommend starting with five ranks.'] = 'ランクを持ち過ぎる必要はありません。5つから始めることをお勧めします。コミュニティが非常に大きい場合、さらに追加することができます。';
...
$Definition['ShareImageDescription'] = '誰かがあなたのサイトからリンクを共有すると、私たちはそのページから画像を取り込もうとします。ページに画像がない場合は、代わりにこの画像を使用します。画像は少なくとも50ｘ50である必要があり、200ｘ200をお勧めします。';
...
$Definition['Some themes may need to be updated to work with category following. You can disable the feature while you update your theme.'] = '一部のテーマは、「カテゴリーフォロイング」で機能するように更新する必要があるかもしれません。テーマの更新中にこの機能を無効にすることができます。';
...
$Definition['Special restriction: Only users with permission Garden.Curation.Manage may use this reaction.'] = '特別な制限：許可「Garden.Curation.Manage」を持つユーザーのみがこのリアクションを使用することができます。';
...
$Definition['Tagging allows users to add a tag to discussions they start in order to make them more discoverable. '] = 'タグ付けを使用すると、ユーザーは開始するディスカッションにタグを追加して、より見つけやすくすることができます。';
...
$Definition['This role is personal info. Only users with permission to view personal info will see it.'] = 'この役割は個人情報です。個人情報の表示許可を持つユーザーのみが個人情報を見られます。';
$Definition['This setting loads the page before executing Javascript.'] = 'この設定は、SEOを改善できるJavascriptを実行する前にページをロードします。<br> <strong> **警告：この機能を有効にすると、サイトでJavascriptエラーが発生する可能性があります。** </strong>';
...
$Definition['TouchIconDescription'] = 'Appleデバイスのホーム画面でウェブサイトをブックマークすると、タッチアイコンが表示されます。これらは通常152ピクセルです。Appleは、角丸と照明効果を自動的に追加します。';
