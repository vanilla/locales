<?php

$Definition['%1$s Version %2$s'] = '%1$s កំណែ %2$s';
$Definition['%1$s version %2$s is available.'] = '%1$s មាន​កំណែ %2$s ហើយ។';
$Definition['%s is not a valid email address'] = '%s មិន​មែន​ជា​អាសយដ្ឋាន​អ៊ីមែល​ត្រឹម​ត្រូវ​ទេ';
$Definition['%s level'] = '%s កម្រិត';
$Definition['%s levels'] = '%s កម្រិត';

$Definition['(less)'] = '(តិច)';
$Definition['(more)'] = '(ច្រើន)';

$Definition['1 day'] = '1 ថ្ងៃ';
$Definition['1 hour'] = '1 ម៉ោង';
$Definition['1 month'] = '1 ខែ';
$Definition['1 month after being sent'] = '1 ខែ​ក្រោយ​ពេល​បាន​ផ្ញើ';
$Definition['1 week'] = '1 សប្ដាហ៍';
$Definition['1 week after being sent'] = '1 សប្ដាហ៍​ក្រោយ​ពេល​បាន​ផ្ញើ';

$Definition['2 weeks after being sent'] = '2 សប្ដាហ៍​ក្រោយ​បាន​ផ្ញើ';

$Definition['4 hours'] = '4 ម៉ោង';

$Definition['5 minutes'] = '5 នាទី';

$Definition['15 minutes'] = '15 នាទី';

$Definition['<strong>Categories</strong>'] = '<strong>ចំណាត់​ក្រុម</strong>';

$Definition['_Locale'] = 'Locale';

$Definition['About Vanilla Statistics'] = 'អំពី​ស្ថិតិ Vanilla';
$Definition['Accepted'] = 'បាន​ព្រម​ទទួល';
$Definition['Action'] = 'សកម្មភាព';
$Definition['Add Info &amp; Create Account'] = 'បន្ថែម​ព័ត៌មាន &amp; បង្កើត​គណនី';
$Definition['API Status'] = 'ស្ថានភាព API';

$Definition['Badge Name'] = 'ឈ្មោះ';

$Definition['Operation By'] = 'ដោយ';

$Definition['Permission.Category'] = 'ចំណាត់​ក្រុម';

$Definition['Saved'] = 'បន្លាស់​ប្ដូរ​របស់​អ្នក​ត្រូវ​បាន​រក្សា​ទុក។';
$Definition['SaveLocalePack'] = 'រក្សា​ទុក';
